...
class TemplateModule(seiscomp.kernel.Module):
    def __init__(self, env):
        ...
        cfg.readConfig(
            os.path.join(self.pkgroot, "etc", "defaults", self.name + ".cfg")
        )
        ...
        cfg.readConfig(
            os.path.join(os.environ["HOME"], ".seiscomp", self.name + ".cfg")
        )

        self.global_params = dict(
            [(x, ",".join(cfg.getStrings(x))) for x in cfg.names()]
        )
    ...
    def _process_template(
        self, tpl_file, source=None, station_scope=True, print_error=True
    ):
        tpl_paths = []
    ...
    def param(self, name, station_scope=True, print_warning=False):
        ...
        if print_warning:
            if station_scope:
                print(
                    "warning: parameter '%s' is not defined for station %s %s"
    ...
    def _get(self, name, station_scope=True):
        try:
            return self.param(name, station_scope)
        except KeyError:
            return ""
...
class Module(TemplateModule):
    ...
    def _run(self):
        ...
        try:
            ...
            print(
                " failed to raise the maximum number open files:",
                str(e),
                file=sys.stderr,
            )
        ...
        if "sequence_file_cleanup" in self.global_params:
            try:
                ...
                if max_minutes > 0:
                    ...
                    for f in files:
                        if (time.time() - os.path.getmtime(f)) / 60 >= max_minutes:
                            print(f" removing sequence file {f}", file=sys.stderr)
                            os.remove(f)
                else:
                ...
                print(
                    " sequence_file_cleanup parameter is not a number: '%s'"
                    % str(self.global_params["sequence_file_cleanup"]),
                    file=sys.stderr,
                )
                return 1

        return self.env.start(
            self.name, self.env.binaryFile(self.name), daemon_opt, not self.env.syslog
        )
    ...
    def _generateStationForIni(self):
        ini = 'station %s  description = "%s"\n' % (
            self._get("seedlink.station.id"),
            self._get("seedlink.station.description"),
        )
        ini += f"             name = \"{self._get('seedlink.station.code')}\"\n"
        ini += f"             network = \"{self._get('seedlink.station.network')}\"\n"
        if self._get("seedlink.station.access"):
            ini += '             access = "%s"\n' % self._get(
                "seedlink.station.access"
            ).replace(",", " ")
        if self._get("seedlink.station.blanks"):
            ini += f"             blanks = \"{self._get('seedlink.station.blanks')}\"\n"
        if self._get("seedlink.station.encoding"):
            ini += '             encoding = "%s"\n' % self._get(
                "seedlink.station.encoding"
            )
        if self._get("seedlink.station.buffers"):
            ini += '             buffers = "%s"\n' % self._get(
                "seedlink.station.buffers"
            )
        if self._get("seedlink.station.segments"):
            ini += '             segments = "%s"\n' % self._get(
                "seedlink.station.segments"
            )
        if self._get("seedlink.station.segsize"):
            ini += '             segsize = "%s"\n' % self._get(
                "seedlink.station.segsize"
            )
        if self._get("seedlink.station.backfill_buffer"):
            ini += '             backfill_buffer = "%s"\n' % self._get(
                "seedlink.station.backfill_buffer"
            )
        if self._get("seedlink.station.sproc"):
            ini += f"             proc = \"{self._get('seedlink.station.sproc')}\"\n"
        ini += "\n"
        return ini

    def __process_station(self, profile):
        if profile:
            self.station_config_file = f"profile_{profile}"
        else:
            self.station_config_file = f"station_{self.net}_{self.sta}"
        ...
        if self._last_net != self.net:
            print(f"+ network {self.net}")
            self._last_net = self.net

        print(f"  + station {self.sta} {description}")
        ...
        for source_type in self._get("sources").split(","):
            ...
            if pluginHandler is None:
                print(f"Error: no handler for plugin {source_type} defined")
                continue
            ...
            if source_type != source_alias:
                ...
                for x in self.station_params.keys():
                    if x.startswith(f"sources.{source_type}."):
                        continue
                    if x.startswith(f"sources.{source_alias}."):
                        toks = x.split(".")
                        toks[1] = source_type
        ...
        if sproc_name:
            self.sproc_used = True
            sproc_filename = f"streams_{sproc_name}.tpl"
    ...
    def __load_stations(self):
        ...
        self._set(
            "seedlink.filters", os.path.join(self.config_dir, "filters.fir"), False
        )
        self._set(
            "seedlink.streams", os.path.join(self.config_dir, "streams.xml"), False
        )
        ...
        for f in files:
            ...
            while line:
                ...
                if not m:
                    print(f"invalid binding in {f}: {line}")
                    line = fd.readline()
                    continue
    ...
    def updateConfig(self):
        ...
        self._set_default(
            "lockfile", os.path.join("@ROOTDIR@", self.env.lockFile(self.name)), False
        )
        self._set_default(
            "filebase",
            os.path.join("@ROOTDIR@", "var", "lib", self.name, "buffer"),
            False,
        )
        ...
        if self.database_str:
            if dbAvailable == True:
                ...
                print(
                    " Database configured but trunk is not installed", file=sys.stderr
                )
                self.seedlink_station_descr = dict()
        ...
        if self.msrtsimul:
            self.seedlink_source["mseedfifo"] = {
                1: ("mseedfifo", 1, self.global_params.copy(), {})
            }
    ...
    def printCrontab(self):
        print(
            "55 23 * * * %s >/dev/null 2>&1"
            % (os.path.join(self.config_dir, "backup_seqfiles"),)
        )
